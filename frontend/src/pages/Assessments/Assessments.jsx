...
export function Assessments() {
  ...
  const loadAssessments = async () => {
    ...
    try {
      ...
      // Laravel paginated response shape: { data: [...], meta: {...} }
              ...
              {assessments.length === 0 ? (
                ...
                assessments.map((assessment) => (
                  <tr key={assessment.id}>
                    <td>
                      <div className="employee-name">{assessment.employee?.name ?? '—'}</div>
                      <div className="employee-email">{assessment.employee?.email}</div>
                    </td>
